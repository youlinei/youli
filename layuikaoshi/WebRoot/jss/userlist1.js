layui.extend({
  ...
  function upUser(userid){
  	...
  	layui.layer.open({
  		title : "修改用户信息",
  		type : 2,
  		content : "/layuikaoshi/jsp/caidan/menuInfo.jsp",
  		area:['800px','800px'],
  		success:function(layero, index){
  			$.ajax({
  				url:"/layuikaoshi/news?action=caidanlistxiugai",
  				type:"post",
  				data:{"userid":userid},
  					...
  					var info = eval('(' + data + ')');
		                ...
		                body.find("#mid").val(info.data.id);
          				...
          				var menuid3 = info.data.mfatherid;
          				 alert(menuid3)
          				//上级的下拉框
             			 $.post("/layuikaoshi/news?action=caidanlistxiugai",{"userid":menuid3},function(res){
         					var cs = eval('(' + res + ')');
  				...
  				$.ajax({
  						  ...
  						  success:function(data){
  							  ...
  							  $.ajax({
  								  url:"/layuikaoshi/news?action=queryUserIsRole",	//查询用户是否有角色有返回1,没有返回0
  								  data:{"userid":userid},
  								  type:"post",
  								  success:function(data){
  									  if(data == 0){
...
/*    //分配角色
  ...
  function addUser(){
  	layui.layer.open({
  		title : "添加用户",
  		type : 2,
  		content : "/layuikaoshi/jsp/caidan/menuAdd.jsp",
  		area:['800px','890px'],
  	})
  }
